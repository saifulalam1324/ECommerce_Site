            ...
            <div class="card-body">
                <ul class="list-group mb-2">
                    @foreach ($batch['items'] as $item)
                        ...
                        <li class="list-group  item d-flex justify-content-between">
                            ...
                            <span>Placed: {{ $batch['updated_at'] }}</span>
                            <span>
                                ${{ number_format($display_price, 2) }}
                                @if($item['line_total'] != $item['discounted_total'])
                                    <small class="text-muted text-decoration-line-through">${{ number_format($item['line_total'], 2) }}</small>
                                @endif
                            </span>
                ...
                <h5 class="text-end">
                    ...
                    @if($batch['batch_total'] != $batch['batch_discounted_total'])
                        <small class="text-muted text-decoration-line-through">${{ number_format($batch['batch_total'], 2) }}</small>
                    @endif
                </h5>

                <form action="{{ route('UpdateDeliveryStatusDone', $batchId) }}" method="POST">
                    @csrf
                    <input type="hidden" name="vendor_email" value="{{ $item['vendor_email'] }}">
                    <button type="submit" class="btn text-white" style="background-color: #081621;">Delivery Done</button>
                </form>
            </div>
        </div>
    @endforeach
